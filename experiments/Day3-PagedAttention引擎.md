...
4. 如果有 → **引用计数 +1**，不分配新块（节省显存）。注意：当前实现的 `prepare_prefill` 仍会对整段 prompt 做完整前向（`model_runner.py:196` 用完整 `token_ids`），所以**省块不省算力**——真正跳过已缓存 token 的计算是 Day8（Chunked Prefill）的事
5. 如果没有 → 分配新块，填满后记录哈希
